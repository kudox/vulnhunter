...
**A real-time local-events platform for AI agents** — multi-source aggregation (ticketing APIs, venue websites, calendar feeds, curated aggregators), cross-source dedup with provenance, price/availability history, and a claimable-promotions layer, exposed as MCP tools.

The server itself is application-neutral. Things built (or plannable) on top of it:

- **Perishable-inventory promotions** (working name: *LastCall* — the business-strategy docs below use this name): merchants post expiring inventory (tonight's empty seats, the 5pm lull, unsold tickets) as targeted, rules-bound offers that agents claim and redeem. Merchants pay only on redemption (12% at confirmation); sponsored placement is bounded and always disclosed (`"sponsored": true`).
- **Composed experiences**: because this is just an MCP server, agents can combine it with other servers — weather + restaurant reservations + `events_search_offers` is a real-time date-night generator; add transit for a full itinerary.
- **Demand intelligence**: the snapshot history (sell-through curves, search demand) is a dataset with standalone analytics value.
...
**Project tracking**: [TASKS.md](./TASKS.md) (backlog/WIP/done) · [WORKLOG.md](./WORKLOG.md) (per-session decisions and gotchas) · [docs/OSINT-EVENT-SOURCING.md](./docs/OSINT-EVENT-SOURCING.md) (free/non-merchant event ingestion plan) · [docs/PAYMENTS-STRATEGY.md](./docs/PAYMENTS-STRATEGY.md) (three-lane payments/monetization decision) · [docs/MERCHANT-PILOT-PLAYBOOK.md](./docs/MERCHANT-PILOT-PLAYBOOK.md) (supply-side pilot: trust ladder, pitch script, agreement template) · [docs/DATABASE-STRATEGY.md](./docs/DATABASE-STRATEGY.md) (Neon decision, scale path, setup runbook).

## Tools

| Tool | What it does |
|---|---|
| `events_search_offers` | Search live events with filters (category, neighborhood, party size, price, time window, min discount, `claimable_only`). Returns claimable **offers** and informational **listings** (see below); ranked by discount depth + urgency; sponsored results labeled. `max_price=0` = free events only. |
| `events_get_offer` | Full details for one offer: description, venue, terms, live availability. |
| `events_claim_offer` | Place a 10-minute hold; spots come off the market immediately and auto-return if not confirmed. |
| `events_confirm_redemption` | Convert a hold into a confirmed booking; returns the door code and the fee-split receipt. Idempotent. |
| `events_release_claim` | Release a hold early; spots return to the pool. |
...
```bash
claude mcp add events -- node /absolute/path/to/events-mcp-server/dist/index.js
```
...
```json
{
  "mcpServers": {
    "events": {
      "command": "node",
      "args": ["/absolute/path/to/events-mcp-server/dist/index.js"]
    }
  }
}
```
...
EVENTS_PG_SSL=on                            # force TLS if the URL lacks sslmode=require
```
...
The repo ships a GitHub Actions schedule (`.github/workflows/events-sync.yml`) that runs it hourly: add `DATABASE_URL` (a free Neon database works) and `TICKETMASTER_API_KEY` as repo Actions secrets, and history starts accumulating for every configured market with zero servers. Note: GitHub only fires `schedule:` triggers from the **default branch** — until the workflow is merged there, trigger it manually from the Actions tab. The worker defaults seed (fictional demo) data **off** so it never pollutes the history tables.
...
- **`listing`** — informational events synced from OSINT sources (free events and non-merchant events included), always attributed with a `source_url` link-out. Not claimable — `events_claim_offer` rejects listings and points the agent at the source. The completeness tier that makes the tool the default answer to "what's happening tonight?" (strategy: [docs/OSINT-EVENT-SOURCING.md](./docs/OSINT-EVENT-SOURCING.md)).
...
The Discovery API has genuinely public search (free tier: 5,000 calls/day). The adapter pulls upcoming events for each configured market — default **San Francisco + Sacramento**, override with `TICKETMASTER_MARKETS="San Francisco,CA;Sacramento,CA"` (or single-market `TICKETMASTER_CITY`/`TICKETMASTER_STATE_CODE`) — within `EVENTS_LISTING_MAX_DAYS_OUT` (14 days), maps segments to categories (Music → live_music, Comedy genre → comedy, Sports → sports, …), and ingests them as listings. Events with no concrete start time or a non-onsale status are skipped with logged reasons; missing price ranges become `priceUnknown` (excluded from any `max_price` search). Re-ingest runs every 60 min (`EVENTS_INGEST_REFRESH_MINUTES`, `0` to disable). Verify without a key: `npm run test:listings`.
...
```bash
EVENTS_JSONLD=on npm start                          # crawl the curated SF venue list
EVENTS_JSONLD=on EVENTS_JSONLD_VENUES=https://... # or your own comma-separated URLs
```

Extracts schema.org `Event` JSON-LD from venue event pages (`src/services/adapters/sfVenues.ts` holds the curated list — adding a venue is adding a URL). Politeness is built in: robots.txt honored per host, identifiable `EventsBot` User-Agent, sequential fetches, per-venue error isolation. Timezone-naive datetimes (a Live Nation quirk) are interpreted as venue-local. Opt-in via `EVENTS_JSONLD=on` since it makes outbound requests to third-party sites. Verify offline: `npm run test:jsonld`.
...
```bash
EVENTS_ICS=on npm start                                    # curated SF community feeds
EVENTS_ICS=on EVENTS_ICS_FEEDS=webcal://...,https://...  # your own feeds
```
...
EVENTBRITE_API_TOKEN=... EVENTS_SEED=off npm start     # real events only
```
...
|---|---|---|
| Discount off face value | 25% | `EVENTS_EB_DISCOUNT_PCT` |
| Claim cutoff before start | 2h | `EVENTS_EB_CLAIM_CUTOFF_HOURS` |
| Only promote events under this sold ratio | 80% | `EVENTS_EB_MAX_SOLD_RATIO` |
| Max spots released per promotion | 40 | `EVENTS_EB_MAX_SPOTS` |
| Ignore events further out than | 14 days | `EVENTS_EB_MAX_DAYS_OUT` |
