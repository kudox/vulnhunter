...
**Decision**: the codebase is a **generic events platform**, not a brand. The user may build multiple applications on it (e.g., composing with weather + reservation MCP servers for a real-time date-night generator alongside another project, Enamor). "LastCall" survives only in the business-strategy docs as the *working name of the promotions application*.

**Renamed** (clean break, nothing deployed): directory `lastcall-mcp-server` → `events-mcp-server`; tools `lastcall_*` → `events_*`; env vars `LASTCALL_*` → `EVENTS_*`; crawler UA → `EventsBot`; redemption codes `LC-` → `EV-`; `LastcallDb` → `EventsDb`; workflow → `.github/workflows/events-sync.yml` ("Events data sync"). GitHub Actions secret names unchanged. All 7 suites green; stdio probe confirms `events-mcp-server` serving `events_*` tools.

**Note for future sessions**: worklog/tasks entries older than this one predate the rename — historical "LastCall"/`lastcall_*` references in them are correct as history. If the hourly sync workflow was already configured before this rename landed, re-check the Actions run after merge (paths changed).
...
- `.github/workflows/events-sync.yml` (repo root): hourly schedule + manual dispatch; needs `DATABASE_URL` and `TICKETMASTER_API_KEY` repo secrets. **GitHub only fires schedules from the default branch** — inert until merged there.
...
- `IcsFeedAdapter` (`ics:<host>` sources, precedence 60): `assumeFree` feeds yield $0 listings (iCalendar has no price field), LOCATION zip regex → neighborhood, `webcal://` normalization, per-feed error isolation. Opt-in via `EVENTS_ICS=on`; `EVENTS_ICS_FEEDS` override.
...
**Shipped**
- `JsonLdCrawlerAdapter`: extracts schema.org `Event` JSON-LD from venue pages. Robots.txt honored per host (minimal `User-agent: *` parser), identifiable `EventsBot` UA, sequential fetches, per-venue error isolation, @graph/ItemList/subEvent walking, stable IDs (`sha256(host|url|startDate)`), Event-subtype → category mapping, relative URL resolution.
- Curated 23-venue SF seed list (`sfVenues.ts`); opt-in via `EVENTS_JSONLD=on` (it fetches third-party sites); `EVENTS_JSONLD_VENUES` override.
...
- Ingest isolates adapter failures (one broken source doesn't sink the sync) and re-syncs each 60 min (`EVENTS_INGEST_REFRESH_MINUTES`).
...
- Pure mapper + promotion rule (`src/services/eventbriteInventory.ts`): decides which events become offers (defaults: 25% off, 2h claim cutoff, skip events ≥80% sold, cap 40 spots, 14-day horizon; `EVENTS_EB_*` env overrides).
- `OfferStore.upsertInventory` re-sync semantics; startup sync + 30-min refresh; `EVENTS_SEED=off`; fixture test suite (`npm run test:eventbrite`) against a fake fetch.
...
- This repo (`vulnhunter`) is otherwise unrelated; LastCall lives entirely under `events-mcp-server/`.
