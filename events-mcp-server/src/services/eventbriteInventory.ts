...
/**
 * Promotion rules: how a merchant's raw Eventbrite inventory becomes the events server
...
export function promotionRuleFromEnv(env: NodeJS.ProcessEnv = process.env): PromotionRule {
  ...
  return {
    discountPct: num("EVENTS_EB_DISCOUNT_PCT", DEFAULT_PROMOTION_RULE.discountPct),
    claimCutoffHours: num("EVENTS_EB_CLAIM_CUTOFF_HOURS", DEFAULT_PROMOTION_RULE.claimCutoffHours),
    maxSoldRatio: num("EVENTS_EB_MAX_SOLD_RATIO", DEFAULT_PROMOTION_RULE.maxSoldRatio),
    maxSpots: num("EVENTS_EB_MAX_SPOTS", DEFAULT_PROMOTION_RULE.maxSpots),
    maxDaysOut: num("EVENTS_EB_MAX_DAYS_OUT", DEFAULT_PROMOTION_RULE.maxDaysOut),
  };
}

/** Eventbrite top-level category IDs -> the events server categories. */
...
/**
 * Map one Eventbrite event (plus its ticket classes) to a the events server offer,
...
export function mapEventToOffer(
  ...
  const offer: Offer = {
    ...
    description: truncate(summary || "Last-minute availability released through this server.", 300),
