...
/** Ticketmaster segment/genre -> the events server category. */
...
export function mapTmEventToListing(
  ...
  const offer: Offer = {
    ...
    sponsored: false,
    terms: `Informational listing from Ticketmaster — tickets are sold at the source, not through this server.${event.url ? ` Buy at: ${event.url}` : ""}`,
  };
