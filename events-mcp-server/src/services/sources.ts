...
export function buildListingAdaptersFromEnv(env: NodeJS.ProcessEnv = process.env): SourceAdapter[] {
  ...
  if (env.EVENTS_JSONLD === "on") {
    const urls = env.EVENTS_JSONLD_VENUES?.split(",")
      .map((u) => u.trim())
      .filter(Boolean);
    const venues = urls
      ? urls.map((url) => ({ url, name: new URL(url).hostname }))
      : CURATED_VENUE_PAGES;
    const maxDaysOut = Number(env.EVENTS_LISTING_MAX_DAYS_OUT ?? "14") || 14;
    adapters.push(new JsonLdCrawlerAdapter({ venues, maxDaysOut }));
  }

  if (env.EVENTS_ICS === "on") {
    const urls = env.EVENTS_ICS_FEEDS?.split(",")
      .map((u) => u.trim())
      .filter(Boolean);
    const feeds = urls
