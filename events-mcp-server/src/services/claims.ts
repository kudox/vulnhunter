...
import type { EventsDb } from "./db.js";
    ...
    try {
      ...
      if (!reserved.ok) {
        ...
        return {
          ok: false,
          reason: "sold_out",
          message: `Only ${reserved.remaining} spot(s) left on '${offer.title}' across the whole marketplace — not enough for a party of ${partySize}.`,
        };
      }
      return local;
    } catch (error) {
