...
export function registerSearchOffers(
  ...
  server.registerTool(
    "events_search_offers",
    {
      title: "Search the events server Offers",
...
Two kinds of results, distinguished by "kind"/"claimable":
  - "offer": claimable through this server (hold -> confirm -> door code); has discount, availability, and a claim_deadline after which it disappears.
  - "listing": informational only — free events and non-merchant events included for completeness; links out to its source for tickets. events_claim_offer rejects listings.
...
Tip: max_price=0 returns free events only. This tool is read-only; claiming happens via events_claim_offer.
...
Returns (JSON format):
  {
    ...
    "offers": [{
      "id": string,                   // pass to events_get_offer / events_claim_offer
      "kind": "offer" | "listing",
      "claimable": boolean,
    ...
    async (params: Input) => {
      try {
        ...
        if (params.response_format === ResponseFormat.MARKDOWN) {
          const lines = [
            `# the events server offers in ${CITY}`,
            "",
            `Found ${total} result(s), showing ${offers.length}. Offers are claimable with \`events_claim_offer\`; [LISTING] entries link out to their source for tickets.`,
            "",
