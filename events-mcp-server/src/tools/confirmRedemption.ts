...
const inputShape = {
  claim: z
    .string()
    .min(1)
    .describe("Claim ID (e.g. 'clm_a1b2c3d4e5') or redemption code (e.g. 'EV-4F7A2B') from events_claim_offer"),
};
...
export function registerConfirmRedemption(
  ...
  server.registerTool(
    "events_confirm_redemption",
    {
      title: "Confirm a the events server Redemption",
...
Returns:
  {
    ...
    "total": string,
    "platform_fee": string,      // the server's ${PLATFORM_FEE_PCT}% take
...
Error handling:
  ...
  - Unknown claim: check the claim_id/redemption code from events_claim_offer`,
