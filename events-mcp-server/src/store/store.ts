...
/**
 * Storage interface for the the events server marketplace.
 *
...
function newRedemptionCode(): string {
  ...
  return `EV-${randomBytes(3).toString("hex").toUpperCase()}`;
}
  ...
  claimOffer(offerId: string, partySize: number, now: Date = new Date()): StoreResult<Claim> {
    ...
    if (!offer) {
      return {
        ok: false,
        reason: "not_found",
        message: `No offer found with id '${offerId}'. Use events_search_offers to find current offers.`,
      };
    }
    if (offer.kind !== "offer") {
      return {
  ...
  confirmClaim(claimIdOrCode: string, now: Date = new Date()): StoreResult<Claim> {
    ...
    if (!claim) {
      return {
        ok: false,
        reason: "not_found",
        message: `No claim found for '${claimIdOrCode}'. Pass the claim_id or redemption code returned by events_claim_offer.`,
      };
    }
