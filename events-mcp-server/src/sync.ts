 ...
 *    event_snapshots history. EVENTS_SEED=on re-enables it.
...
async function main(): Promise<void> {
  ...
  if (adapters.length > 0) {
    ...
    console.log(
      "listings: skipped (no sources configured — set TICKETMASTER_API_KEY, EVENTS_JSONLD=on, EVENTS_ICS=on, EVENTS_FUNCHEAP=on)",
    );
  }
