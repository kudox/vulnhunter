#!/usr/bin/env node
/**
 * Events MCP server — a perishable-inventory promotion marketplace.
 *
...
import { EventsDb } from "./services/db.js";
import {
...
function makeStore(): InMemoryOfferStore {
  const seed = process.env.EVENTS_SEED !== "off";
...
): Promise<{ analytics: Analytics; db?: EventsDb }> {
 ...
 * re-sync on an interval (default 60 min, EVENTS_INGEST_REFRESH_MINUTES;
...
async function startListingIngest(store: OfferStore, analytics: Analytics): Promise<void> {
  ...
  if (adapters.length === 0) {
    console.error(
      "Listing ingest disabled (no sources configured — set TICKETMASTER_API_KEY, EVENTS_JSONLD=on, EVENTS_ICS=on, and/or EVENTS_FUNCHEAP=on)",
    );
    return;
  }
  ...
  const refreshMinutes = Number(process.env.EVENTS_INGEST_REFRESH_MINUTES ?? "60");
  if (Number.isFinite(refreshMinutes) && refreshMinutes > 0) {
    const timer = setInterval(() => {
      sync().catch((error) => {
...
async function runStdio(): Promise<void> {
  ...
  console.error("Events MCP server running via stdio");
}
...
async function runHttp(): Promise<void> {
  ...
  app.listen(port, host, () => {
    console.error(`Events MCP server running on http://${host}:${port}/mcp`);
  });
}
