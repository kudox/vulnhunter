...
export function funcheapConfigFromEnv(env: NodeJS.ProcessEnv = process.env): FuncheapConfig | undefined {
  if (env.LASTCALL_FUNCHEAP !== "on") return undefined;
  return {
    baseUrl: env.LASTCALL_FUNCHEAP_BASE ?? "https://sf.funcheap.com",
...
export function mapFuncheapEvent(
  ...
  const offer: Offer = {
    ...
    sponsored: false,
    terms: `Informational listing curated by Funcheap SF — details at the source, not through LastCall. ${sourceUrl}${ticketUrl ? ` Tickets: ${ticketUrl}` : ""}`,
  };
