...
const USER_AGENT =
  "LastCallBot/0.1 (local events aggregation; respects robots.txt)";
/** Schema.org Event subtypes we recognize, mapped to LastCall categories. */
...
export function mapLdEventToListing(
  ...
  const offer: Offer = {
    ...
    sponsored: false,
    terms: `Informational listing from the venue's website — tickets at the source, not through LastCall.${sourceUrl ? ` Details: ${sourceUrl}` : ""}`,
  };
