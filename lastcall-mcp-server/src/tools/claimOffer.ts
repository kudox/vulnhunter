...
const inputShape = {
  offer_id: z
    .string()
    .min(1)
    .describe("Offer ID from lastcall_search_offers (e.g. 'off_fogline_tonight')"),
  party_size: z
    .number()
    .int()
    .min(1)
...
Flow: claim -> present price and terms to the user -> lastcall_confirm_redemption to lock it in (or lastcall_release_claim to give the spots back early).
...
Returns:
  {
    ...
    "status": "held",
    "party_size": number,
    "redemption_code": string,   // e.g. "LC-4F7A2B" — shown at the door after confirmation
    ...
    async (params: Input) => {
      try {
        ...
        const text = [
          ...
          "",
          `Confirm with \`lastcall_confirm_redemption\` before the hold expires, or release with \`lastcall_release_claim\`.`,
        ].join("\n");
