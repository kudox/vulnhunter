...
const inputShape = {
  offer_id: z
    .string()
    .min(1)
    .describe("Offer ID from lastcall_search_offers (e.g. 'off_velvet_late_set')"),
  response_format: z
...
export function registerGetOffer(server: McpServer, store: OfferStore): void {
  server.registerTool(
    "lastcall_get_offer",
    {
      title: "Get LastCall Offer Details",
...
Use after lastcall_search_offers when the user wants specifics before committing, or to re-check availability of a known offer.
...
Error handling:
  - "Error: No offer found..." if the ID is unknown — re-run lastcall_search_offers; offers vanish permanently after their claim deadline.`,
    ...
    async (params: Input) => {
      try {
        ...
        if (!offer) {
          return toolError(
            `No offer found with id '${params.offer_id}'. Offers disappear after their claim deadline — use lastcall_search_offers to see what's live now.`,
          );
        }
